...
const GovernanceTab = ({ contracts, account }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup function: component is unmounting
    };
  ...
  const handleCreateSnapshot = async () => {
    if (!contracts.justToken || !isMounted.current) return;
      ...
      if (receipt.status === 1) {
        ...
        try {
          ...
          if (snapshotEvent) {
            ...
            if (isMounted.current) setTxStatus('Snapshot created successfully!');
          }
        } catch (e) {
          if (isMounted.current) setTxStatus('Snapshot created successfully!');
        }
        ...
        if (isMounted.current) setTxStatus('Transaction failed');
      }
    } catch (error) {
      if (isMounted.current) {
  ...
  const handleRescueETH = async () => {
    ...
    try {
      // Get contract address - this is the problematic line, safely handle different ethers versions
      let contractAddress;
      if (contracts.justToken.address) {
        contractAddress = contracts.justToken.address;
      } else if (contracts.justToken.target) {
        contractAddress = contracts.justToken.target;
      } else {
        // If neither property exists, we need a fallback
        // We could use the contract's connection to get its address
        const tx = await contracts.justToken.getAddress();
        contractAddress = tx;
      }
  ...
  const handleUpdateGovParam = async (paramType, newValue, paramName) => {
    if (!contracts.governance || !isMounted.current) return;
