...
export function useProposals() {
  ...
  const fetchProposals = useCallback(async () => {
    ...
    try {
      ...
      safeSetState(setProposals, processedProposals);
      ...
      safeSetState(setTokenHolders, uniqueProposers.size);
      
    } catch (err) {
      console.error("Error fetching proposals:", err);
      if (isMountedRef.current) {
        safeSetState(setError, "Failed to fetch proposals: " + err.message);
      }
    } finally {
      if (isMountedRef.current) {
        safeSetState(setLoading, false);
  ...
  const createProposal = async (
    ...
    try {
      safeSetState(setLoading, true);
      safeSetState(setError, null);
      safeSetState(setCreateProposalStatus, {
        isSubmitting: true,
        error: null,
        success: false
      });
      ...
      // Check if component is still mounted after the async operations
      if (!isMountedRef.current) return;
      ...
      // Check component mounted state again
      if (!isMountedRef.current) return;
  ...
  useEffect(() => {
    ...
    // Return cleanup function that runs when the component unmounts
    return () => {
  ...
  }, [fetchProposals, isConnected, contractsReady, refreshCounter, account, safeSetState]);
